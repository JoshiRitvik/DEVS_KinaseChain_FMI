...
#define MODEL_IDENTIFIER KinaseChain
#define MODEL_GUID "{35d61dbc-871a-46fc-b014-8652b7a50b9c}"
...
#define FMI2_FUNCTION_PREFIX KinaseChain_
...
extern void KinaseChain_setupDataStruc(DATA *data, threadData_t *threadData);
#define fmu2_model_interface_setupDataStruc KinaseChain_setupDataStruc
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 12 && (comp->state == model_state_instantiated || comp->state == model_state_initialization_mode)) {
